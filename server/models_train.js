      ...
      .on("data", (row) => {
        ...
        if (!isNaN(qty)) {
          ...
          if (!inventoryData[key]) {
            inventoryData[key] = { data: [], labels: [] };
          }
          inventoryData[key].data.push([month]);
          inventoryData[key].labels.push(qty);
        }
      })
      .on("end", () => {
...
setTimeout(() => {
  ...
  Object.keys(inventoryData).forEach((key) => {
    const { data, labels } = inventoryData[key];
    if (data.length < 2) {
    ...
    const regression = new ml.SLR(data, labels);
    ...
    data.forEach((month, index) => {
      ...
      const predicted = regression.predict(month);
      if (actual !== 0) {
        totalPercentageError += Math.abs((actual - predicted) / actual) * 100;
      }
    });
    const mape = totalPercentageError / labels.length;
